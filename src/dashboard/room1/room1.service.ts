...
import { room1Document } from './schema/room1.schema';

@Injectable()
export class Room1Service {
  private readonly room1Keys = ['U7_Active_Energy_Total_Consumed'];

  constructor(
    @InjectModel('room1')
    private readonly room1Model: Model<room1Document>,
  ) {}

  async handleQuery(query: Getroom1Dto) {
    switch (query.value) {
      case 'today':
  ...
  async getTodayData() {
    const collection = this.room1Model.collection;
    ...
    this.room1Keys.forEach((key) => (projection[key] = 1));
    ...
    for (let h = 0; h < 24; h++) {
      ...
      for (const key of this.room1Keys) {
