...
import { room2Document } from './schema/room2.schema';
import { Getroom2Dto } from './dto/get-room2.dto';

@Injectable()
export class Room2Service {
  private readonly room2Keys = ['U8_Active_Energy_Total_Consumed'];

  constructor(
    @InjectModel('room2')
    private readonly room2Model: Model<room2Document>,
  ) {}

  async handleQuery(query: Getroom2Dto) {
    switch (query.value) {
      case 'today':
  ...
  async getTodayData() {
    const collection = this.room2Model.collection;
    ...
    this.room2Keys.forEach((key) => (projection[key] = 1));
    ...
    for (let h = 0; h < 24; h++) {
      ...
      for (const key of this.room2Keys) {
