...
import { room6Document } from './schema/room6.schema';
import { Getroom6Dto } from './dto/get-room6.dto';

@Injectable()
export class Room6Service {
  private readonly room6Keys = ['U12_Active_Energy_Total_Consumed'];

  constructor(
    @InjectModel('room6')
    private readonly room6Model: Model<room6Document>,
  ) {}

  async handleQuery(query: Getroom6Dto) {
    switch (query.value) {
      case 'today':
  ...
  async getTodayData() {
    const collection = this.room6Model.collection;
    ...
    this.room6Keys.forEach((key) => (projection[key] = 1));
    ...
    for (let h = 0; h < 24; h++) {
      ...
      for (const key of this.room6Keys) {
