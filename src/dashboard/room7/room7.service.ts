...
import { room7Document } from './schema/room7.schema';
import { Getroom7Dto } from './dto/get-room7.dto';

@Injectable()
export class Room7Service {
  private readonly room7Keys = ['U6_Active_Energy_Total_Consumed'];

  constructor(
    @InjectModel('room7')
    private readonly room7Model: Model<room7Document>,
  ) {}

  async handleQuery(query: Getroom7Dto) {
    switch (query.value) {
      case 'today':
  ...
  async getTodayData() {
    const collection = this.room7Model.collection;
    ...
    this.room7Keys.forEach((key) => (projection[key] = 1));
    ...
    for (let h = 0; h < 24; h++) {
      ...
      for (const key of this.room7Keys) {
