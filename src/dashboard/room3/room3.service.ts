...
import { room3Document } from './schema/room3.schema';

@Injectable()
export class Room3Service {
  private readonly room3Keys = ['U9_Active_Energy_Total_Consumed'];

  constructor(
    @InjectModel('room3')
    private readonly room3Model: Model<room3Document>,
  ) {}

  async handleQuery(query: Getroom3Dto) {
    switch (query.value) {
      case 'today':
  ...
  async getTodayData() {
    const collection = this.room3Model.collection;
    ...
    this.room3Keys.forEach((key) => (projection[key] = 1));
    ...
    for (let h = 0; h < 24; h++) {
      ...
      for (const key of this.room3Keys) {
