...
import { room4Document } from './schema/room4.schema';
import { Getroom4Dto } from './dto/get-room4.dto';

@Injectable()
export class Room4Service {
  private readonly room4Keys = ['U10_Active_Energy_Total_Consumed'];

  constructor(
    @InjectModel('room4')
    private readonly room4Model: Model<room4Document>,
  ) {}

  async handleQuery(query: Getroom4Dto) {
    switch (query.value) {
      case 'today':
  ...
  async getTodayData() {
    const collection = this.room4Model.collection;
    ...
    this.room4Keys.forEach((key) => (projection[key] = 1));
    ...
    for (let h = 0; h < 24; h++) {
      ...
      for (const key of this.room4Keys) {
        ...
        const { before: yBeforeStart, after: yAfterStart } = findBoundingDocs(
          dataByDay.Yesterday,
          yStart,
        );
        const { before: yBeforeEnd, after: yAfterEnd } = findBoundingDocs(
          dataByDay.Yesterday,
          yEnd,
        );

        const valStartYesterday = interpolateValue(
          yStart,
          yBeforeStart,
          yAfterStart,
          key,
        );
        const valEndYesterday = interpolateValue(
          yEnd,
          yBeforeEnd,
          yAfterEnd,
          key,
        ...
        const { before: tBeforeStart, after: tAfterStart } = findBoundingDocs(
          dataByDay.Today,
          tStart,
        );
        const { before: tBeforeEnd, after: tAfterEnd } = findBoundingDocs(
          dataByDay.Today,
          tEnd,
        );

        const valStartToday = interpolateValue(
          tStart,
          tBeforeStart,
          tAfterStart,
          key,
  ...
  async getWeekData() {
    ...
    for (const doc of data) {
      ...
      const weekLabel =
        date >= startOfThisWeek && date <= endOfThisWeek
          ? 'This Week'
          : 'Last Week';
    ...
    for (const day of daysOfWeek) {
      ...
      for (const key of this.room4Keys) {
        if (
          ...
          thisWeekTotal +=
            lastValues[day]['This Week'][key] -
            firstValues[day]['This Week'][key];
        }
        ...
        if (
          ...
          lastWeekTotal +=
            lastValues[day]['Last Week'][key] -
            firstValues[day]['Last Week'][key];
        }
      }
  ...
  async getYearData() {
    ...
    try {
      ...
      const output = months.map((month) => ({
        Month: month,
        'Current Year': this.formatNumber(
          yearlyConsumption[currentYear][month],
        ),
        'Previous Year': this.formatNumber(
          yearlyConsumption[previousYear][month],
        ),
      }));
