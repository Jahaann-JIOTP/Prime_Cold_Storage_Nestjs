...
export interface EnergyUsageResult {  // Exporting the interface
    date: string;
    meterId: string;
    consumption: number;
    startValue: number;
    ...
    async getEnergyUsage(dto: EnergyUsageDto): Promise<EnergyUsageResult[]> {
        ...
        for (let d = new Date(start); d < end; d.setDate(d.getDate() + 1)) {
            ...
            for (const meterId of meterIds) {
                ...
                if (firstDoc && lastDoc) {
                    const startVal = firstDoc.get(key) ?? 0;
                    const endVal = lastDoc.get(key) ?? 0;
